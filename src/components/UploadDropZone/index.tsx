...
import { useDrive, getUploadFolderPath, UserID } from "@officexapp/framework";
...
const UploadDropZone: React.FC<UploadDropZoneProps> = ({ children, toggleUploadPanel }) => {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    if (acceptedFiles.length > 0) {
      ...
      const directoryStructure = acceptedFiles.reduce((acc, file) => {
      ...
      for (const directory in directoryStructure) {
        ...
        uploadFilesFolders(
          ...
          5,
          (fileUUID) => {
            console.log(`Local callback: File ${fileUUID} in ${directory} uploaded successfully`);
          }
        );
      }
  ...
  return (
    <div
      {...getRootProps({ className: `custom-upload-dropzone ${isDragActive ? 'drag-active' : ''}` })}
