...
export const createPseudoShareLink = async ({
    ...
    try {
        ...
        const shareLink = `https://drive.officex.app/gift?id=${data.id}&ref=${ref}`;
        return shareLink;
    } catch (error) {
...
export const getPseudoShareLink = async (id: string): Promise<{ url: string, title: string }> => {
    const body = {
        route: 'GET/gift',
        payload: {
            id
...
export function updateRefInShareUrl(originalUrl: string, newRef: string): string {
