...
const DocumentEditor = () => {
  ...
  const fileType = "officex-document";
  ...
  useEffect(() => {
    ...
    if (file.id !== lastLoadedFileRef.current) {
      const defaultName =
        file.name.replace(".officex-document", "") || "Unknown File";
      setCurrentFileName(defaultName);
      currentFileNameRef.current = defaultName;
    }
  ...
  const saveFileContent = async (fileContent: string) => {
    ...
    try {
      ...
      const fileObject = new File(
        [blob],
        `${_currentFileName.replace(".officex-document", "")}.officex-document`,
        {
          type: "application/json",
        }
      );
            ...
            updateFileAction(
              {
                ...
                payload: {
                  id: file.id,
                  name: `${_currentFileName.replace(".officex-document", "")}.officex-document`,
                },
              },
              undefined,
              shouldBehaveOfflineDiskUIIntent(file.disk_id)
  ...
  const parentMethods = {
    ...
    downloadFile: (fileContent: string) => {
      ...
      a.href = url;
      const _fileName = `${_currentFileName.replace(".officex-document", "")}.officex-document`;
  ...
  return (
    <div>
      <Helmet>
        <meta charSet="utf-8" />
        <title>{currentFileName.replace(".officex-document", "")}</title>
